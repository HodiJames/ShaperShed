      ...
      <div className="modal">
        ...
        {mode === "in" ? <>
          ...
          <form onSubmit={sub}>
            ...
            <div style={{textAlign:"right",marginBottom:8}}><span style={{fontSize:12,color:"var(--g)",cursor:"pointer"}} onClick={() => alert("Password reset feature coming soon. Please contact hello@shapershed.com for assistance.")}>{tr("auth.forgotPassword")}</span></div>
          ...
          {sorted.map(q => (
            <div key={q.id} className="ask-q">
              ...
              <div className="ask-q-body">
                ...
                <div className="ask-q-meta">
                  {q.name} · {q.date}
                  {q.boardName && <span className="ask-q-board">🏄 {q.boardName}</span>}
                </div>
              </div>
            </div>
          ))}
...
function PremiumKnowledgeGrid({ items }) {
  const [open, setOpen] = useState(null);
  return (
    <div className="ld-knowledge">
      {items.map((item, i) => (
        <div key={item.id || i} className={`ld-kcard ${open === i ? "open" : ""}`} onClick={() => setOpen(open === i ? null : i)}>
          <div className="ld-kcard-top">
            <div className="ld-kcard-icon">📖</div>
            <div className="ld-kcard-topic">{item.topic}</div>
            <span className="ld-kcard-arrow">▼</span>
          </div>
          {open === i && (
            <div className="ld-kcard-body">
              {item.description}
              {item.videoUrl && (
                <div style={{ marginTop: 12 }}>
                  <a href={item.videoUrl} target="_blank" rel="noopener noreferrer" style={{ color: "var(--g)", fontSize: 13 }}>
                    📹 Watch related video ↗
                  </a>
                </div>
              )}
...
// ─────────────────────────────────────────────
// BOARD ASK QUESTION COMPONENT
// ─────────────────────────────────────────────
function BoardAskQuestion({ listing, boardName, onSuccess }) {
  const { user, setModal, showToast } = useContext(Ctx);
    ...
    try {
      const res = await fetch(`${API_BASE}/api/questions`, {
        ...
        body: JSON.stringify({
          shaperId: listing.id,
          shaperName: listing.name,
          boardName: boardName, // Include board context
          text: question,
          authorName: name || "Anonymous",
          authorEmail: user.email,
          votes: 0,
          source: "board_portfolio" // Track source
        })
      });
      ...
      if (res.ok) {
        ...
        setName("");
        setIsOpen(false);
        showToast("Question submitted! It will appear once reviewed.");
        if (onSuccess) onSuccess();
      } else {
        showToast("Failed to submit question. Please try again.");
      }
    } catch (err) {
  ...
  return (
    <div className="board-ask">
      <button 
        className="board-ask-toggle" 
        onClick={() => setIsOpen(!isOpen)}
      >
        <span>💬 Ask about this board</span>
        <span className="board-ask-arrow">{isOpen ? "▲" : "▼"}</span>
      </button>
      
      {isOpen && (
        <div className="board-ask-form">
          <textarea
            className="board-ask-input"
            placeholder={`What would you like to know about the ${boardName}?`}
            value={question}
            onChange={(e) => setQuestion(e.target.value)}
            rows={2}
          />
          <div className="board-ask-row">
            <input
              className="board-ask-name"
              placeholder="Your name (optional)"
              value={name}
              onChange={(e) => setName(e.target.value)}
            />
            <button 
              className="board-ask-submit"
          ...
          <p className="board-ask-note">
            Questions are reviewed before appearing in the Ask section
          </p>
        </div>
      )}
    </div>
...
// ─────────────────────────────────────────────
function BoardImageCarousel({ images, boardName }) {
  ...
  return (
    <>
      <div className="board-carousel" onClick={() => setLightboxOpen(true)}>
        <img 
          src={images[currentIndex].url} 
          alt={`${boardName} - Image ${currentIndex + 1}`} 
          className="board-carousel-img"
        />
        {images.length > 1 && (
          <>
            <button className="board-carousel-btn prev" onClick={prev}>‹</button>
            <button className="board-carousel-btn next" onClick={next}>›</button>
            <div className="board-carousel-dots">
              {images.map((_, i) => (
                <span 
                  key={i} 
                  className={`board-carousel-dot ${i === currentIndex ? "active" : ""}`}
...
function ListingPage({ listing }) {
  ...
  const premiumVideos = listing.premiumContent?.videos || [];
  const premiumKnowledge = listing.premiumContent?.knowledge || [];
  const premiumBoards = listing.premiumContent?.boards || [];
  ...
  return (
    <div className="ldp">
            ...
            <div className="ld-hright">
              ...
              {isOwner && isPremium && (
                <button 
                  ...
                  style={{ marginTop: 8 }}
                  onClick={() => { setSelected(listing); setPage("edit-premium"); }}
                >
                  Edit Content
                </button>
              )}
      ...
      <div className="ld-body">
        ...
        {/* User-added Premium Videos */}
        {isPremium && premiumVideos.length > 0 && (
          <div className="ld-sec">
            <div className="ld-sec-title">🎬 Watch This Shaper at Work <span>{tr("listing.premium")}</span></div>
            {premiumVideos.map((v, i) => {
              const videoId = v.url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([^&\?\/]+)/)?.[1];
              return (
                <div key={v.id || i} style={{ marginBottom: i < premiumVideos.length - 1 ? 20 : 0 }}>
                  {videoId ? (
                    <div className="ld-video">
                      <iframe
                    ...
                    <a href={v.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--g)" }}>{v.url}</a>
                  )}
                  {v.title && <div className="ld-vtitle">"{v.title}"</div>}
                </div>
              );
            })}
          </div>
        ...
        {/* User-added Shaping Knowledge */}
        {isPremium && premiumKnowledge.length > 0 && (
          <div className="ld-sec">
            <div className="ld-sec-title">📚 Shaping Knowledge <span>{tr("listing.premium")}</span></div>
            <p style={{ fontSize: 13, color: "var(--txm)", marginBottom: 16 }}>
              How {listing.name.split(" ")[0]} thinks about the fundamentals.
            </p>
            <PremiumKnowledgeGrid items={premiumKnowledge} />
          </div>
        )}
          ...
          <div className="ld-sec">
            ...
            <div className="ld-boards">
              ...
              {listing.boards?.map((b, i) => (
                <div key={`orig-${i}`} className="ld-board">
                  ...
                  <BoardAskQuestion listing={listing} boardName={b.name} />
                </div>
              ))}
              {/* User-added boards from premiumContent */}
              {premiumBoards.map((b, i) => (
                <div key={`premium-${b.id || i}`} className="ld-board">
                  {b.images?.length > 0 && (
                    <BoardImageCarousel images={b.images} boardName={b.name} />
                  )}
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8 }}>
                    <div className="ld-board-name">{b.name}</div>
                    {b.type && <span className="ld-board-type">{b.type}</span>}
                  </div>
                  <div className="ld-board-specs">
                    {b.length && <span className="ld-board-spec">📏 {b.length}</span>}
                    {b.fins && <span className="ld-board-spec">🏄 {b.fins}</span>}
                  </div>
                  {b.description && <div className="ld-board-desc">{b.description}</div>}
                  {b.price && (
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:8 }}>
                      <div className="ld-board-price">{b.price}</div>
                    </div>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {[
                ...
                { val:"Repairs", icon:"🔧", label:tr("sub.repairs"), desc:tr("sub.repairsDesc") },
              ].map(opt => (
...
function ProfilePage() {
  const { user, setUser, setPage, setSelected, listings, savedIds, showToast, tr } = useContext(Ctx);
...
// ─────────────────────────────────────────────
// PREMIUM EDIT PAGE - For owners to manage premium content
// ─────────────────────────────────────────────
function PremiumEditPage() {
  const { user, selected, setPage, setListings, showToast } = useContext(Ctx);
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState({
    // Watch This Shaper at Work - YouTube videos
    videos: selected?.premiumContent?.videos || [],
    // Shaping Knowledge - Educational content
    knowledge: selected?.premiumContent?.knowledge || [],
    // Board Portfolio
    boards: selected?.premiumContent?.boards || []
  });
  ...
  // Board input state  
  const [newBoard, setNewBoard] = useState({ name: "", type: "", length: "", fins: "", price: "", description: "", url: "", images: [] });
  ...
  if (!user || !selected || selected.ownerEmail !== user.email) {
    return (
      <div className="prof-wrap">
        <div className="empty">
          <div className="emico">🔒</div>
          <p>You don't have permission to edit this listing.</p>
  ...
  if (!selected.premium) {
    return (
      <div className="prof-wrap">
        <div className="empty">
          <div className="emico">💎</div>
          <p>Upgrade to Premium to add custom content.</p>
          <button className="btn bp" onClick={() => setPage("listing")}>Go to Listing</button>
        </div>
      </div>
    );
  }
  ...
  const addVideo = () => {
    ...
    setForm(f => ({ ...f, videos: [...f.videos, { ...newVideo, id: Date.now() }] }));
  ...
  const addKnowledge = () => {
    if (!newKnowledge.topic.trim()) return;
    setForm(f => ({ ...f, knowledge: [...f.knowledge, { ...newKnowledge, id: Date.now() }] }));
    setNewKnowledge({ topic: "", description: "", videoUrl: "" });
  };

  const removeKnowledge = (id) => {
    setForm(f => ({ ...f, knowledge: f.knowledge.filter(k => k.id !== id) }));
  };

  const addBoard = () => {
    if (!newBoard.name.trim()) return;
    setForm(f => ({ ...f, boards: [...f.boards, { ...newBoard, id: Date.now() }] }));
    setNewBoard({ name: "", type: "", length: "", fins: "", price: "", description: "", url: "", images: [] });
  };
  ...
  const handleBoardImageUpload = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const result = await uploadImage(file, `boards/${selected.id}`, (progress) => {
          setUploadProgress(Math.round(((i + progress / 100) / files.length) * 100));
        });
        uploadedImages.push(result);
      }
      
      setNewBoard(p => ({ ...p, images: [...p.images, ...uploadedImages] }));
      showToast(`${uploadedImages.length} image(s) uploaded!`);
    } catch (err) {
    ...
    setUploadProgress(0);
    if (boardImageRef.current) boardImageRef.current.value = '';
  };

  const removeNewBoardImage = async (index) => {
    const img = newBoard.images[index];
    if (img.path) {
      try {
    ...
    setNewBoard(p => ({ ...p, images: p.images.filter((_, i) => i !== index) }));
  };

  const removeBoard = (id) => {
    setForm(f => ({ ...f, boards: f.boards.filter(b => b.id !== id) }));
  };

  const saveChanges = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/listings/${selected.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          ...selected, 
      ...
      if (res.ok) {
        ...
        setListings(prev => prev.map(l => l.id === selected.id ? { ...l, premiumContent: form } : l));
        showToast("Premium content saved successfully!");
      } else {
  ...
  return (
    <div className="prof-wrap" style={{ maxWidth: 900 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <button className="ld-backbtn" onClick={() => setPage("profile")}>← Back to My Listings</button>
        <button className="btn bp" onClick={saveChanges} disabled={loading}>
          {loading ? "Saving..." : "Save Changes"}
        </button>
      </div>

      <div style={{ background: "var(--sf)", border: "1px solid var(--bd)", borderRadius: "var(--rl)", padding: 28, marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 8 }}>
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, margin: 0 }}>{selected.name}</h1>
          <span className="my-listing-badge premium">Premium</span>
        </div>
        <p style={{ color: "var(--tx2)", fontSize: 14 }}>Manage your premium content to showcase your craft to surfers worldwide.</p>
      </div>

      {/* SECTION: Watch This Shaper at Work */}
      <div className="ld-sec" style={{ marginTop: 0 }}>
        <h3 className="ld-sec-title">🎬 Watch This Shaper at Work</h3>
        <p style={{ fontSize: 13, color: "var(--tx2)", marginBottom: 16 }}>Add YouTube videos showing your shaping process.</p>
        
        {form.videos.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
            {form.videos.map(v => (
              <div key={v.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--bg)", border: "1px solid var(--bdl)", borderRadius: "var(--rs)" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{v.title || "Untitled Video"}</div>
                  <div style={{ fontSize: 12, color: "var(--txm)", marginTop: 2 }}>{v.url}</div>
                </div>
                <button className="btn bxs brej" onClick={() => removeVideo(v.id)}>✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          />
          <button className="btn bp bsm" onClick={addVideo}>+ Add Video</button>
        </div>
      </div>

      {/* SECTION: Shaping Knowledge */}
      <div className="ld-sec">
        <h3 className="ld-sec-title">📚 Shaping Knowledge</h3>
        <p style={{ fontSize: 13, color: "var(--tx2)", marginBottom: 16 }}>Share educational content about board design features.</p>
        
        {form.knowledge.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
            {form.knowledge.map(k => (
              <div key={k.id} style={{ padding: "14px 16px", background: "var(--bg)", border: "1px solid var(--bdl)", borderRadius: "var(--rs)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{k.topic}</div>
                    {k.description && <div style={{ fontSize: 13, color: "var(--tx2)", marginTop: 4 }}>{k.description}</div>}
                    {k.videoUrl && <div style={{ fontSize: 12, color: "var(--g)", marginTop: 4 }}>📹 {k.videoUrl}</div>}
                  </div>
                  <button className="btn bxs brej" onClick={() => removeKnowledge(k.id)}>✕</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <input 
            className="fi" 
            placeholder="Topic (e.g., 'Understanding Rocker')" 
            value={newKnowledge.topic} 
            onChange={e => setNewKnowledge(p => ({ ...p, topic: e.target.value }))}
          />
          <textarea 
            className="ft" 
            placeholder="Description of this topic..." 
            value={newKnowledge.description} 
            onChange={e => setNewKnowledge(p => ({ ...p, description: e.target.value }))}
            rows={2}
          />
          <div style={{ display: "flex", gap: 10 }}>
            <input 
              ...
              value={newKnowledge.videoUrl} 
              onChange={e => setNewKnowledge(p => ({ ...p, videoUrl: e.target.value }))}
              style={{ flex: 1 }}
            />
            <button className="btn bp bsm" onClick={addKnowledge}>+ Add Topic</button>
          </div>
        </div>
      </div>

      {/* SECTION: Board Portfolio */}
      <div className="ld-sec">
        <h3 className="ld-sec-title">🏄 Board Portfolio</h3>
        <p style={{ fontSize: 13, color: "var(--tx2)", marginBottom: 16 }}>Showcase your boards with specs and pricing.</p>
        
        {form.boards.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12, marginBottom: 16 }}>
            {form.boards.map(b => (
              <div key={b.id} style={{ padding: "16px", background: "var(--bg)", border: "1px solid var(--bdl)", borderRadius: "var(--r)" }}>
                {b.images?.length > 0 && (
                  <div style={{ display: "flex", gap: 6, marginBottom: 12, overflowX: "auto", paddingBottom: 4 }}>
                    {b.images.map((img, i) => (
                      <img key={i} src={img.url} alt={`${b.name} ${i+1}`} style={{ width: 80, height: 60, objectFit: "cover", borderRadius: 6, flexShrink: 0 }} />
                    ))}
                  </div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div style={{ fontWeight: 700, fontSize: 15 }}>{b.name}</div>
                  <button className="btn bxs brej" onClick={() => removeBoard(b.id)} style={{ padding: "2px 6px", fontSize: 12 }}>✕</button>
                </div>
                {b.type && <span className="ld-board-type">{b.type}</span>}
        ...
        <div style={{ background: "var(--bg)", border: "1px solid var(--bdl)", borderRadius: "var(--r)", padding: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <input className="fi" placeholder="Board Name *" value={newBoard.name} onChange={e => setNewBoard(p => ({ ...p, name: e.target.value }))} />
            <input className="fi" placeholder="Type (e.g., Fish, Longboard)" value={newBoard.type} onChange={e => setNewBoard(p => ({ ...p, type: e.target.value }))} />
            <input className="fi" placeholder="Length Range (e.g., 5'6&quot;-6'0&quot;)" value={newBoard.length} onChange={e => setNewBoard(p => ({ ...p, length: e.target.value }))} />
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 8 }}>📷 Board Images</label>
            {newBoard.images.length > 0 && (
              <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                {newBoard.images.map((img, i) => (
                  <div key={i} style={{ position: "relative" }}>
                    <img src={img.url} alt={`Upload ${i+1}`} style={{ width: 80, height: 60, objectFit: "cover", borderRadius: 6 }} />
                    <button 
                      onClick={() => removeNewBoardImage(i)} 
                      style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: "50%", background: "#ef4444", color: "#fff", border: "none", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                    >✕</button>
                  </div>
                ))}
              </div>
            ...
            <button 
              ...
              style={{ width: "100%" }}
            >
              {uploadingImages ? `Uploading... ${uploadProgress}%` : `+ Add Images${newBoard.images.length > 0 ? ` (${newBoard.images.length} added)` : ""}`}
            </button>
            <p style={{ fontSize: 11, color: "var(--txm)", marginTop: 6 }}>Upload up to 5 images per board. Max 5MB each. JPG, PNG, WebP.</p>
          </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button className="btn bp bsm" onClick={addBoard} disabled={!newBoard.name.trim()}>+ Add Board</button>
          </div>
        </div>
      </div>

      <div style={{ marginTop: 24, textAlign: "center" }}>
        <button className="btn bp" style={{ padding: "14px 40px", fontSize: 15 }} onClick={saveChanges} disabled={loading}>
...
export default function App() {
  ...
  const ctx = {
    ...
    showToast, selected, setSelected, categories, setCategories,
    ...
    <Ctx.Provider value={ctx}>
      ...
      <div className="shell" dir={localeObj.dir}>
        ...
        <main style={{ flex:1, overflowY:"auto" }}>
          ...
          {page==="edit-premium" && <PremiumEditPage />}
        </main>
